...
class MahasiswaService {
    ...
    FakultasRepository $fakultas, ProgramStudiRepository $programstudi, MahasiswaRepository $mahasiswa, UserRepository $user, RoleRepository $role
    ) {
    ...
    public function getProgramStudi($id) {
        $action = '<a href="' . url('mahasiswa/programstudi/' . $id) . '" type="button" class="btn cur-p btn-success">
                Atur Mahasiswa
            </a>';
        return $action;
    }
    ...
    public function getMatkul($id) {
        $action = '<a href="' . url('mahasiswa/mahasiswa/' . $id) . '" type="button" class="btn cur-p btn-success">
                Atur Mahasiswa
            </a>';
        return $action;
    }
    ...
    public function getList($id) {
        $data = $this->mahasiswa->getList($id);
        return Datatables::of($data)
                        ->addColumn('action', function ($data) {
                            $action = $this->getShow($data->id);
                            if (checkHakAkses(["M06.03"])) {
                                $action .= $this->getEdit($data->id);
                            }
                            if (checkHakAkses(["M06.04"])) {
                                if ($data->status == 0) {
    ...
    private function getShow($id) {
        $action = '<a href="' . url('mahasiswa/show/' . $id) . '" title="Detail" class="btn cur-p btn-secondary m-3">
    ...
    private function getEdit($id) {
        $action = '<a href="' . url('mahasiswa/getDataUpdate/' . $id) . '"  class="btn cur-p btn-primary m-3">
