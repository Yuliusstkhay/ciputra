...
class FakultasService {
    ...
    public function __construct(FakultasRepository $fakultas) {
        $this->fakultas = $fakultas;
    }

    public function getList() {
        $data = $this->fakultas->getList();
        return Datatables::of($data)
                        ->addColumn('action', function ($data) {
                            $action = $this->getShow($data->fakultas_id);
                            if (checkHakAkses(["M02.03"])) {
                                $action .= $this->getEdit($data->fakultas_id);
                            }
                            if (checkHakAkses(["M02.04"])) {
                                if ($data->status == 0) {
                                    $action .= $this->getVoid($data->fakultas_id);
                                } else {
                                    $action .= $this->getUnvoid($data->fakultas_id);
                                }
                            }
                            return $action;
                        })
    ...
    private function getEdit($id) {
        $action = '<a href="' . url('fakultas/getData/' . $id) . '"  class="btn cur-p btn-primary m-3">
